# CLAUDE.md — SimpleKernel

> This file mirrors AGENTS.md for Claude Code auto-loading. See AGENTS.md and sub-module AGENTS.md files for full details.
...
| `src/include/kernel.h` | Singleton aliases, global includes | header-only |
...
| `src/include/kernel_fdt.hpp` | Device tree parser | header-only |
| `src/include/kernel_elf.hpp` | ELF parser | header-only |
| `src/include/kernel_log.hpp` | Logging (MPMC queue, levels) | header-only |
| `src/include/spinlock.hpp` | Spinlock | header-only |
...
> See also `docs/coding_standards.md` for full prohibited-patterns list.
...
- **Sub-module AGENTS.md files**: When working in `src/arch/`, `src/device/`, `src/task/`, `src/filesystem/`, or `tests/`, read the corresponding `AGENTS.md` for module-specific guidance.
