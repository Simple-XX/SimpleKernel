...
void print_MULTIBOOT_TAG_TYPE_APM(multiboot_tag_t *tag) {
    printk_info("APM type 0x%X, Size 0x%X, version 0x%X, cseg 0x%X, offset "
                "0x%X, cseg_16 0x%X, "
    ...
    local_intr_store(intr_flag);
    {
        ...
        for (tag = (multiboot_tag_t *)tag_addr;
            ...
            case MULTIBOOT_TAG_TYPE_ELF_SECTIONS: {
                ...
                // 0) { 		kernel_elf.strtab = (const char *)shdr[i].sh_addr;
                // 		// printk("strtab: 0x%X\n", kernel_elf.strtab);
                // 		// printk("strtab: %s\n", kernel_elf.strtab);
                ...
                // kernel_elf.symtab); 		kernel_elf.symtabsz = shdr[i].sh_size;
                // 		// printk("symtab->: 0x%X\n",
