...
pte_t* VirtualMemoryManager::find(const pt_t _pgd, uintptr_t _va, bool _alloc) {
  ...
  for (size_t level = cpu::vmm_info::VMM_PT_LEVEL - 1; level > 0; level--) {
    ...
    if ((*pte & cpu::vmm_info::VMM_PAGE_VALID) == 1) {
      // pgd 指向下一级页表
    ...
    // 如果无效
      ...
      if (_alloc == true) {
        ...
        bzero(pgd, PhysicalMemoryManager::kPageSize);
        // 申请失败则返回
        if (pgd == nullptr) {
        ...
        bzero(pgd, PhysicalMemoryManager::kPageSize);
        // 填充页表项
        *pte = PA2PTE((uintptr_t)pgd) | cpu::vmm_info::VMM_PAGE_VALID;
      }
      // 不分配的话直接返回
      else {
        return nullptr;
...
VirtualMemoryManager& VirtualMemoryManager::get_instance(void) {
  /// 定义全局 VirtualMemoryManager 对象
  static VirtualMemoryManager vmm;
  return vmm;
}

bool VirtualMemoryManager::init(void) {
#if defined(__i386__) || defined(__x86_64__)
  GDT::init();
#endif
  // 分配一页用于保存页目录
  pt_t pgd_kernel = (pt_t)PMM::get_instance().alloc_page_kernel();
  bzero(pgd_kernel, PhysicalMemoryManager::kPageSize);
  // 映射内核空间
  for (uintptr_t addr = (uintptr_t)COMMON::KERNEL_START_ADDR;
    ...
    mmap(pgd_kernel, addr, addr,
         cpu::vmm_info::VMM_PAGE_READABLE | cpu::vmm_info::VMM_PAGE_WRITABLE |
             cpu::vmm_info::VMM_PAGE_EXECUTABLE);
  }
  // 设置页目录
  set_pgd(pgd_kernel);
  // 开启分页
...
pt_t VirtualMemoryManager::get_pgd(void) { return (pt_t)CPU::GET_PGD(); }
...
void VirtualMemoryManager::mmap(const pt_t _pgd, uintptr_t _va, uintptr_t _pa,
  ...
  // 没有映射，或更改了 flag
    ...
    *pte = PA2PTE(_pa) | _flag |
           (*pte & ((1 << cpu::vmm_info::VMM_PTE_PROP_BITS) - 1)) |
           cpu::vmm_info::VMM_PAGE_VALID;
    // 刷新缓存
    CPU::VMM_FLUSH(0);
  }
...
void VirtualMemoryManager::unmmap(const pt_t _pgd, uintptr_t _va) {
  ...
  // 未找到
  if (pte == nullptr) {
    warn("VirtualMemoryManager::unmmap: find.\n");
    return;
  }
  // 找到了，但是并没有被映射
  if ((*pte & cpu::vmm_info::VMM_PAGE_VALID) == 0) {
    warn("VirtualMemoryManager::unmmap: not mapped.\n");
  }
  // 置零
  *pte = 0x00;
  // 刷新缓存
...
bool VirtualMemoryManager::get_mmap(const pt_t _pgd, uintptr_t _va,
  ...
  bool res = false;
  // pte 不为空且有效，说明映射了
  if ((pte != nullptr) && ((*pte & cpu::vmm_info::VMM_PAGE_VALID) == 1)) {
    // 如果 _pa 不为空
    if (_pa != nullptr) {
      // 设置 _pa
      // 将页表项转换为物理地址
...
uint32_t VirtualMemoryInit(uint32_t, uint8_t*) {
  // 初始化虚拟内存管理器
  kVirtualMemoryManager.GetInstance() =
      VirtualMemoryManager(kBasicInfo.GetInstance().physical_memory_addr,
