...
int vsprintf(char *buf, const char *fmt, va_list args) {
    ...
    int32_t
        precision; // min. # of digits for integers;max number of chars for from
    // fields.  min.整数数字个数；max. 字符串中字符个数
    int32_t qualifier
        __attribute__((unused)); // 'h','l',or 'L' for integer fields
    ...
    repeat:
        ...
        // 取精度值。若此时宽度值小于 0，则将字段精度值取为 0. get the precision
        precision = -1;
        if (*fmt == '.') {
            ++fmt;
            if (is_digit(*fmt))
        ...
        case 'p':
            ...
            break;
        // 若格式转换指示是 'x'  或
        // 'X'，则表示对应参数需打印成十六进制数输出。'x' 表示用小写字母表示。
        case 'x':
            flags |= SMALL;
