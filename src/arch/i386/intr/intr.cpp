...
void irq_handler(pt_regs_t *regs) {
    intrk.call_irq(regs->int_no, regs);
    return;
}

// ISR 处理函数
void isr_handler(pt_regs_t *regs) {
    intrk.call_isr(regs->int_no, regs);
    return;
}
...
void die(const char *str, uint32_t oesp, uint32_t int_no) {
    ...
    // old_esp->old_esp);
    iok.printf("gs: %08x\tfs: %08x\tes: %08x\tds: %08x\n", old_esp->gs,
               old_esp->fs, old_esp->es, old_esp->ds);
    iok.printf("edi: %08x\tesi: %08x\tebp: %08x\told_esp: %08x\n", old_esp->edi,
               old_esp->esi, old_esp->ebp, old_esp->old_esp);
    iok.printf("ebx: %08x\tedx: %08x\tecx: %08x\teax: %08x\n", old_esp->ebx,
               old_esp->edx, old_esp->ecx, old_esp->eax);
    iok.printf("int_no: %08X\terr_code: %08X\teip: %08x\tcs: %08x\n",
               old_esp->int_no, old_esp->err_code, old_esp->eip, old_esp->cs);
    iok.printf("eflags: %08x\tuser_esp: %08x\tss: %08x\n", old_esp->eflags,
               old_esp->user_esp, old_esp->user_ss);
    iok.printf("addr: %08x, %08X\n", &old_esp->gs, &old_esp->user_ss);
...
void INTR::debug(pt_regs_t *regs) {
    ...
    iok.printf("Unuseable.\n");

    iok.printf("eax 0x%08X\tebx 0x%08X\tecx 0x%08X\tedx 0x%08X\n", regs->eax,
...
const char *INTR::get_intr_name(uint32_t intrno) {
...
void INTR::enable_irq(uint32_t irq_no) {
    ...
    if (irq_no >= IRQ8) {
        mask = ((iok.inb(IO_PIC2C)) & (~(1 << (irq_no % 8))));
        iok.outb(IO_PIC2C, mask);
    }
    else {
        mask = ((iok.inb(IO_PIC1C)) & (~(1 << (irq_no % 8))));
        iok.outb(IO_PIC1C, mask);
    }
    return;
}
...
uint32_t INTR::get_irq(uint32_t irq_no) {
    uint32_t res = 0;
    switch (irq_no) {
        case 0: {
            res = IRQ0;
...
void INTR::init_interrupt_chip(void) {
    ...
    // 初始化主片、从片
    // 0001 0001
    iok.outb(IO_PIC1, 0x11);
    // 设置主片 IRQ 从 0x20(32) 号中断开始
    iok.outb(IO_PIC1C, intrk.get_irq(0));
    // 设置主片 IR2 引脚连接从片
    iok.outb(IO_PIC1C, 0x04);
    // 设置主片按照 8086 的方式工作
    iok.outb(IO_PIC1C, 0x01);

    iok.outb(IO_PIC2, 0x11);
    // 设置从片 IRQ 从 0x28(40) 号中断开始
    iok.outb(IO_PIC2C, intrk.get_irq(8));
    // 告诉从片输出引脚和主片 IR2 号相连
    iok.outb(IO_PIC2C, 0x02);
    // 设置从片按照 8086 的方式工作
    iok.outb(IO_PIC2C, 0x01);

    // 默认关闭所有中断
    iok.outb(IO_PIC1C, 0xFF);
    iok.outb(IO_PIC2C, 0xFF);
    return;
}

void INTR::clear_interrupt_chip(uint32_t intr_no) {
    // 发送中断结束信号给 PICs
    // 按照我们的设置，从 32 号中断起为用户自定义中断
    // 因为单片的 Intel 8259A 芯片只能处理 8 级中断
    // 故大于等于 40 的中断号是由从片处理的
    if (intr_no >= intrk.get_irq(8)) {
        // 发送重设信号给从片
        iok.outb(IO_PIC2, PIC_EOI);
    }
    // 发送重设信号给主片
    iok.outb(IO_PIC1, PIC_EOI);
    return;
}

int32_t INTR::call_irq(uint32_t intr_no, pt_regs_t *regs) {
    // 重设PIC芯片
...
int32_t INTR::call_isr(uint32_t intr_no, pt_regs_t *regs) {
    if (interrupt_handlers[intr_no] != nullptr) {
        interrupt_handlers[intr_no](regs);
    }
    else {
