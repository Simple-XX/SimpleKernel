...
// 输入引脚有信号时，那么数学协处理器 x87 错误将被忽略。 当NE为复位状态并且 CPU
// 的 IGNNE 输入引脚无信号时，那么非屏蔽的数学协处理器 x87 错误将导致处理器通过
// FERR 引脚在外部产生一个中断， 并且在执行下一个等待形式浮点指令或 WAIT/FWAIT
// 指令之前立刻停止指令执行。 CPU 的 FERR 引脚用于仿真外部协处理器 80387 的
// ERROR 引脚， 因此通常连接到中断控制器输入请求引脚上。NE 标志、IGNNE 引脚和
// FERR 引脚用于利用外部逻辑来实现 PC 形式的外部错误报告机制。
...
// 32bits 模式的分页模式支持物理地址宽度超过 32bit，这种情况下的页面大小必须是
// 4MB,并且处理器必须支持 PSE-3
