...
//
// task.c for SimpleXX/SimpleKernel.
...
static task_pcb_t *alloc_task_pcb(void) {
    ...
    assert(task_pcb != NULL,
           "Error at task.c: alloc_task_pcb. No enough memory!\n");
    ...
    task_pcb->pt_regs =
        (pt_regs_t *)((ptr_t)task_pcb->mm->stack_bottom - sizeof(pt_regs_t));
...
void task_init(void) {
    ...
    local_intr_store(intr_flag);
    {
        ...
        assert((ptr_t)kernel_task == KERNEL_STACK_TOP,
               "kernel_task not correct\n");
        ...
        // 设置上下文
        kernel_task->context =
            (task_context_t *)kmalloc(sizeof(task_context_t));
...
pid_t do_fork(uint32_t flags __UNUSED__, pt_regs_t *pt_regs) {
    assert(curr_task_count < TASK_MAX,
           "Error: task.c curr_task_count >= TASK_MAX");
...
task_pcb_t __attribute__((regparm(3))) *
    __switch_to(task_pcb_t *curr, task_pcb_t *next) {
