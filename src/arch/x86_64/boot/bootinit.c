...
void mm_init() {
	pgd_tmp[0] = (ptr_t)pte_low | PAGE_PRESENT | PAGE_WRITE;

	for (int i = 0; i < 4; ++i) {
		uint32_t pgd_idx = PGD_INDEX(PAGE_OFFSET + PAGE_MAP_SIZE * i);
		pgd_tmp[pgd_idx] = ( (ptr_t)pte_high + PAGE_SIZE * i ) | PAGE_PRESENT | PAGE_WRITE;
	}
	pgd_tmp[0] = ( (ptr_t)pte_high ) | PAGE_PRESENT | PAGE_WRITE;
