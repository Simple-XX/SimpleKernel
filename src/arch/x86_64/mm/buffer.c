...
//哈希表
struct buffer_head *hash_table[hash_num];
//请求项数组
struct request all_request[request_num];
//当前存在的请求项
...
{
    //缓冲区清零
    bzero(BUFFER_START,BUFFER_END-BUFFER_START);
    //初始化缓冲块结构体
    struct buffer_head *h=(struct buffer_head *)BUFFER_START;
    //每一页对应一个缓冲块，对每个缓冲块进行初始化,结果为1015.07，说明最多包含1015个缓冲块
    int num=(BUFFER_END-BUFFER_START+1)/(sizeof(struct buffer_head)+PMM_PAGE_SIZE);
    //缓冲块存储区起始地址则从结构体后一页开始,
    ptr_t buffer_mem_start=BUFFER_START+((num*sizeof(struct buffer_head))/PMM_PAGE_SIZE+1)*PMM_PAGE_SIZE;
    printk_info("%d\n",sizeof(struct buffer_head));
    printk_info("%d\n",num);
    for(ptr_t i=buffer_mem_start&PMM_PAGE_MASK;i<=(BUFFER_END&PMM_PAGE_MASK);i+=PMM_PAGE_SIZE)
    {
        //每个缓冲块的起始地址
        h->addr=i;
        //每个缓冲块对应的设备块号
...
struct buffer_head *buffer_read(uint8_t dev,uint32_t block)
{
    ...
    memset(bh->addr,0x11111111,PMM_PAGE_SIZE);
...
bool add_request(uint8_t dev,uint8_t cmd,uint32_t sector)
{
    ...
    for(int i=0;i<request_num;i++)
    {
        ...
        if(!all_request[i].dev)
        {
            ...
            current_request=(struct request *)(&all_request[i]);
