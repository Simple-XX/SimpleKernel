...
#undef DEBUG
    ...
    for (uint64_t i = VMM_PGD_INDEX(COMMON::KERNEL_BASE);
...
void VMM::mmap(const page_dir_t pgd, const void *va, const void *pa,
               const uint32_t flag) {
    uint64_t     pgd_idx = VMM_PGD_INDEX(reinterpret_cast<uint64_t>(va));
    ...
    if (pt == nullptr) {
        ...
        pgd[pgd_idx] = (page_dir_entry_t)(reinterpret_cast<uint64_t>(pt) |
