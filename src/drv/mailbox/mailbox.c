...
int32_t mbox_call(uint8_t ch) {
    ...
    do {
        asm volatile("nop");
    } while (*((uint32_t *)MAILBOX_STATUS) & MAILBOX_FULL);
    // 向指定通道写数据
    *((uint32_t *)MAILBOX_WRITE) = r;
    // 等待答复
    while (1) {
        // 先看有没有答复
        do {
            asm volatile("nop");
        } while (*((uint32_t *)MAILBOX_STATUS) & MAILBOX_EMPTY);
        // 判断是不是需要的答复
        if (r == *((uint32_t *)MAILBOX_READ)) {
            // 判断是否有效
            return mbox[1] == MAILBOX_RESPONSE_SUCCESSFUL;
        }
    }
    return 0;
}
