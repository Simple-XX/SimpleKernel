...
// TODO: 完善
bool DTB::get_clint(BOOT_INFO::iter_data_t *_iter_data, void *_data) {
    // 找到 memory 属性节点
    if (strncmp((char *)_iter_data->node_name, "clint", sizeof("clint") - 1) ==
        0) {
        // 找到地址信息
        if (strcmp((char *)_iter_data->prop_name, "reg") == 0) {
            resource_t *resource = (resource_t *)_data;
...
// TODO: 完善
bool DTB::get_plic(BOOT_INFO::iter_data_t *_iter_data, void *_data) {
    // 找到 memory 属性节点
    if (strncmp((char *)_iter_data->node_name, "plic", sizeof("plic") - 1) ==
        0) {
        // 找到地址信息
        if (strcmp((char *)_iter_data->prop_name, "reg") == 0) {
            resource_t *resource = (resource_t *)_data;
...
resource_t BOOT_INFO::get_clint(void) {
    resource_t resource;
    DTB::dtb_iter(DTB::get_clint, &resource);
    return resource;
}

resource_t BOOT_INFO::get_plic(void) {
    resource_t resource;
    DTB::dtb_iter(DTB::get_plic, &resource);
    return resource;
}
