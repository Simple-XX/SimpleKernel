...
DTB::find_compatible(mystl::string _compatible) {
    mystl::vector<mystl::vector<resource_t>> res;
    // 遍历 nodes
    for (size_t i = 0; i < nodes[0].count; i++) {
        // 在节点中找到 compatible 属性
        if (find_prop(&nodes[i], "compatible", _compatible) == true) {
            resource_t resource;
            // 遍历 nodes[i] 匹配属性
            // 中断
            dtb_iter(DT_ITER_PROP, ttt, &resource, &nodes[i]);
            // 内存
            // 其它
        }
    }
...
namespace BOOT_INFO {
    ...
    find_compatible(mystl::string _compatible) {
        return DTB::find_compatible(_compatible);
    }
};
