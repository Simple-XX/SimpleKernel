...
namespace STL {
    ...
    void string::resize(size_t n, char c) {
        if (n < size()) {
            ...
            auto      newCapacity =
                oldCapacity + STL::max(oldCapacity, lengthOfInsert);
            // iterator  newStart =
    ...
    string &string::insert(size_t pos, const string &str, size_t subpos,
        ...
        insert(begin() + pos, str.begin() + subpos,
    ...
    string::iterator string::insert_aux_filln(iterator p, size_t n,
                                              value_type c) {
        // auto     newCapacity = getNewCapacity(n);
        size_type oldCapacity = endOfStorage_ - start_;
        auto      newCapacity = oldCapacity + STL::max(oldCapacity, n);
        iterator  newStart    = dataAllocator::allocate(newCapacity);
    ...
    string &string::replace(size_t pos, size_t len, const string &str) {
        return replace(begin() + pos, begin() + pos + len, str.begin(),
                       str.end());
    }
    ...
    size_t string::find_first_not_of(const char *s, size_t pos,
                                     size_t n) const {
        /*for (size_t i = pos; i != pos + n; ++i){
        if (!isContained((*this)[i], s, s + strlen(s)))
        return i;
    ...
    bool operator<=(const string &lhs, const string &rhs) {
        return lessEqual_aux(lhs.cbegin(), lhs.cend(), rhs.cbegin(),
                             rhs.cend());
    }
    bool operator<=(const char *lhs, const string &rhs) {
    ...
    bool operator>=(const string &lhs, const string &rhs) {
        return greaterEqual_aux(lhs.cbegin(), lhs.cend(), rhs.cbegin(),
                                rhs.cend());
    }
    bool operator>=(const char *lhs, const string &rhs) {
        return greaterEqual_aux(lhs, lhs + strlen(lhs), rhs.cbegin(),
                                rhs.cend());
    }
    bool operator>=(const string &lhs, const char *rhs) {
        return greaterEqual_aux(lhs.cbegin(), lhs.cend(), rhs,
                                rhs + strlen(rhs));
    }
