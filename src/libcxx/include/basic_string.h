...
namespace mystl {
    ...
    struct char_traits<char> {
        ...
        static char_type *fill(char_type *dst, char_type ch,
                               size_t count) noexcept {
            return static_cast<char_type *>(memset(dst, ch, count));
        }
    };
    ...
    //                            size_t n) noexcept {
    //         MYSTL_DEBUG(src + n <= dst || dst + n <= src);
    //         return static_cast<char_type *>(wmemcpy(dst, src, n));
    //     }

    //     static char_type *move(char_type *dst, const char_type *src,
    //                            size_t n) noexcept {
    //         return static_cast<char_type *>(wmemmove(dst, src, n));
    //     }

    //     static char_type *fill(char_type *dst, char_type ch,
    //                            size_t count) noexcept {
    //         return static_cast<char_type *>(wmemset(dst, ch, count));
    //     }
    // };
...
#define STRING_INIT_SIZE 32
    ...
    public:
        ...
        basic_string &replace(const_iterator first, const_iterator last,
            ...
            MYSTL_DEBUG(begin() <= first && last <= end() && first <= last);
            return replace_fill(first, static_cast<size_type>(last - first),
                                count, ch);
        }
        ...
        basic_string &replace(size_type pos1, size_type count1,
            ...
            THROW_OUT_OF_RANGE_IF(
                ...
                "basic_string<Char, Traits>::replace's pos out of range");
            return replace_cstr(buffer_ + pos1, count1, str.buffer_ + pos2,
                                count2);
        }
    ...
    // template <class CharType, class CharTraits>
    // basic_string<CharType, CharTraits> &&
    // basic_string<CharType, CharTraits>::operator=(const char *s) {
    ...
    // template <class CharType, class CharTraits>
    // basic_string<CharType, CharTraits> &&
    // basic_string<CharType, CharTraits>::operator=(char c) {
    //     destroyAndDeallocate();
    ...
    template <class CharType, class CharTraits>
    basic_string<CharType, CharTraits> &
    basic_string<CharType, CharTraits>::append(const_pointer s,
                                               size_type     count) {
        THROW_LENGTH_ERROR_IF(size_ > max_size() - count,
                              "basic_string<Char, Tratis>'s size too big");
        if (cap_ - size_ < count) {
    ...
    // 从 pos1 下标开始的 count1 个字符跟另一个 basic_string 下标 pos2 开始的
    // count2 个字符比较
    template <class CharType, class CharTraits>
    ...
    void basic_string<CharType, CharTraits>::try_init() noexcept {
        try {
            buffer_ = data_allocator::allocate(
                static_cast<size_type>(STRING_INIT_SIZE));
            size_ = 0;
            cap_  = 0;
        } catch (...) {
            buffer_ = nullptr;
    ...
    // fill_init 函数
    template <class CharType, class CharTraits>
    void basic_string<CharType, CharTraits>::fill_init(size_type  n,
                                                       value_type ch) {
    ...
    template <class Iter>
    void
    basic_string<CharType, CharTraits>::copy_init(Iter first, Iter last,
    ...
    void basic_string<CharType, CharTraits>::reinsert(size_type size) {
        ...
        } catch (...) { data_allocator::deallocate(new_buffer); }
        buffer_ = new_buffer;
        size_   = size;
        cap_    = size;
    }
    ...
    basic_string<CharType, CharTraits>::replace_cstr(const_iterator first,
        ...
        if (count1 < count2) {
            ...
            char_traits::move(r + count2, first + count1,
                              end() - (first + count1));
    ...
    template <class CharType, class CharTraits>
    typename basic_string<CharType, CharTraits>::iterator
    basic_string<CharType, CharTraits>::reallocate_and_copy(
